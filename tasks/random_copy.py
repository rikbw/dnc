...
class RandomCopy(snt.AbstractModule):
  ...
  def __init__(self,
               ...
               batch_size=16,
               max_flag_index=10):
    super(RandomCopy, self).__init__(name='RepeatSequence')

    self._nb_vecs = nb_vecs
    self._nb_bits = nb_bits
    self._batch_size = batch_size
    self._max_flag_index = max_flag_index
  ...
  def _build(self):
    ...
    flag_index = randint(1, self._max_flag_index)

    flag_index = tf.random.uniform(
      [1],
      minval=1,
      maxval=self._max_flag_index,
      dtype=tf.int32,
    )[0]
    ...
    for batch_index in range(0, self._batch_size):
      ...
      # Pattern
      obs_pattern = tf.cast(
        tf.random_uniform([self._nb_vecs, self._nb_bits], minval=0, maxval=2, dtype=tf.int32),
        tf.float32
      )
      ...
      observation_padded = tf.concat([
        obs_pattern,
        tf.zeros([flag_index+1, self._nb_bits], dtype=tf.float32)
      ], 0)

      # Create flag channel
      flag_channel_left = tf.zeros([flag_index, 1], dtype=tf.float32)
      flag = tf.ones([1, 1], dtype=tf.float32)
      flag_channel_right = tf.zeros([self._nb_vecs, 1], dtype=tf.float32)

      flag_channel = tf.concat([flag_channel_left, flag, flag_channel_right], 0)

      obs = tf.concat([flag_channel, observation_padded], 1)

      # TARGET

      # Add zeros (padding) to the target
      target_padding = tf.zeros([flag_index+1, self.target_size], dtype=tf.float32)

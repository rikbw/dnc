...
class ReverseCopy(snt.AbstractModule):

  def __init__(self, nb_vecs, vec_length, batch_size, every_third=False):
    super(ReverseCopy, self).__init__(name='Flip3rd')
  ...
  def _build(self):
    ...
    for batch_index in range(0, self._batch_size):
      ...
      # Reverse the pattern
      obs_pattern_reversed = tf.reverse(obs_pattern, [0])

      target = tf.concat([target_padding, obs_pattern_reversed], 0)
